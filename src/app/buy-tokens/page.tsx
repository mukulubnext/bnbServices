  ...
  return (
    <div className="bg-light w-screen py-[10vh] h-screen">
      ...
      <div className="bg-white gap-4 mx-auto w-[96%] px-[2%] py-6 flex flex-col rounded-lg min-h-full">
        <h1 className="font-bold text-dark text-xl">Buy Tokens</h1>
        ...
        <div className="flex flex-col gap-2">
          <h1 className="font-bold text-dark text-xl">Custom Token Value: </h1>
          <div className="flex gap-1 justify-start relative items-center">
            <Coins className="text-dark left-2 absolute"/>
            <input inputMode="numeric" placeholder="Enter tokens" onFocus={()=> {setSelected(0)}} value={custom === 0 ? undefined : custom} onChange={(e)=> setCustom(Number(e.target.value))} type="number" min={200} className='border focus:outline-0 w-full md:w-auto p-3 pl-10 rounded border-dark' />
          </div>
        </div>
